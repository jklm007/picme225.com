...
.pm-results-count { 
    ...
    <section class="pm-mkt-filters">
        <div class="container">
            <form action="{{ route('marketplace.public') }}" method="GET" id="pm-filter-form">
                <!-- Search and Toggle Button -->
                <div style="display:flex; gap:10px; align-items:center;">
                    <div class="pm-search-box" style="flex: 1; min-width: 0;">
                        <i class="fa fa-search"></i>
                        <input type="text" name="search" id="pm-search-input" value="{{ request('search') }}" placeholder="{{ __('home.search_item_placeholder') }}" />
                    </div>
                    <button type="button" onclick="document.getElementById('pm-advanced-filters').style.display = document.getElementById('pm-advanced-filters').style.display === 'none' ? 'flex' : 'none';" style="padding: 12px 16px; background: rgba(255,255,255,0.08); color: var(--text-color); border: 1px solid rgba(255,255,255,0.15); border-radius: 12px; cursor: pointer; white-space: nowrap; font-weight: 600;">
                        <i class="fa fa-sliders"></i> @lang('home.filters')
                    </button>
                </div>
                
                <!-- Advanced Filters -->
                <div id="pm-advanced-filters" style="display: {{ request('city') || request('min_price') || request('max_price') || (request('sort') && request('sort') != 'newest') ? 'flex' : 'none' }}; flex-wrap:wrap; gap:10px; align-items:center; margin-top: 12px; padding-top: 12px; border-top: 1px solid var(--border-color);">
                    <input type="text" name="city" value="{{ request('city') }}" placeholder="{{ __('home.city') }}" style="flex: 1 1 120px; padding: 12px; border-radius: 12px; border: 1px solid var(--border-color); background: var(--bg-card); color: var(--text-color); outline: none;">
                    
                    <input type="number" name="min_price" value="{{ request('min_price') }}" placeholder="{{ __('home.min_price') }}" style="flex: 1 1 100px; padding: 12px; border-radius: 12px; border: 1px solid var(--border-color); background: var(--bg-card); color: var(--text-color); outline: none;">
                    <input type="number" name="max_price" value="{{ request('max_price') }}" placeholder="{{ __('home.max_price') }}" style="flex: 1 1 100px; padding: 12px; border-radius: 12px; border: 1px solid var(--border-color); background: var(--bg-card); color: var(--text-color); outline: none;">
                    ...
                    <select name="sort" style="flex: 1 1 150px; padding: 12px; border-radius: 12px; border: 1px solid var(--border-color); background: var(--bg-card); color: var(--text-color); outline: none;" onchange="document.getElementById('pm-filter-form').submit();">
                        <option value="newest" {{ request('sort') == 'newest' ? 'selected' : '' }}>@lang('home.newest')</option>
                        <option value="price_asc" {{ request('sort') == 'price_asc' ? 'selected' : '' }}>@lang('home.price_asc')</option>
                        <option value="price_desc" {{ request('sort') == 'price_desc' ? 'selected' : '' }}>@lang('home.price_desc')</option>
                    </select>

                    <button type="submit" style="padding: 12px 24px; background: #C9A84C; color: white; font-weight: bold; border: none; border-radius: 12px; cursor: pointer; flex: 1 1 100%;">@lang('home.apply_filters')</button>
                </div>
            </form>
            
            <div class="pm-cat-chips">
                <div class="pm-cat-chip main-cat {{ !request('category') ? 'active-all' : '' }}" data-cat="">@lang('home.show_all')</div>
    ...
    <div class="pm-app-modal" id="pm-app-modal">
        <div class="pm-modal-box">
            ...
            <h3>@lang('home.book_via_app')</h3>
            <p>@lang('home.book_via_app_desc')</p>
            <div class="pm-modal-dl-btns">
                <a href="{{ Setting::get('store_link_android','https://play.google.com/store/apps') }}" target="_blank" class="pm-modal-btn pm-modal-btn-play" id="btn-modal-play">
                    <i class="fa fa-android"></i> @lang('home.google_play')
                </a>
                <a href="javascript:void(0)" onclick="installPWA('{{ Setting::get('store_link_android','#') }}')" class="pm-modal-btn pm-modal-btn-apk" id="btn-modal-apk">
                    <i class="fa fa-download"></i> @lang('home.install_app')
                </a>
            </div>
        </div>
    </div>
    ...
    <section class="pm-mkt-body">
        <div class="container">
            <p class="pm-results-count" id="pm-count">@lang('home.loading_ads')</p>
...
(function() {
    ...
    function renderListings(listings) {
        ...
        if (!listings.length) {
            grid.innerHTML = '<div class="pm-empty" style="grid-column:1/-1;"><i class="fa fa-search"></i><h4>" + "{{ __('home.no_ads_found') }}" + "</h4><p>" + "{{ __('home.try_other_category') }}" + "</p></div>';
    ...
    try {
        ...
        document.getElementById('pm-grid').innerHTML = '<div class="pm-empty" style="grid-column:1/-1;"><i class="fa fa-exclamation-circle" style="color:#FF4B7C;"></i><h4>" + "{{ __('home.error_loading_ads') }}" + "</h4><p>" + "{{ __('home.check_connection') }}" + "</p></div>';
