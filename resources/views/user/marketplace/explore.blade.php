@extends('user.layout.user_dashboard', ['active' => 'store'])
...
.pm-filter-modal.show { display: flex; animation: pm-fadeIn .2s; }
.pm-filter-content {
    background: var(--bg-card);
    width: 100%;
    max-width: 500px;
    ...
    padding: 30px;
    animation: pm-slideUp .3s cubic-bezier(0.165, 0.84, 0.44, 1);
    position: relative;
    max-height: 90vh;
    overflow-y: auto;
}
@media(min-width: 768px) {
    .pm-filter-modal { align-items: center; }
    .pm-filter-content { border-radius: 24px; max-height: 80vh; }
}
...
<div class="pm-mkt">
    <!-- ═══ HEADER ═══ -->
    <div class="pm-mkt-header" style="background: linear-gradient(135deg, var(--lime, #22C55E), var(--lime-dark, #15803D)); padding: 90px 16px 24px 16px; border-bottom: 1px solid rgba(0,0,0,0.08); display: flex; flex-direction: column; gap: 16px;">
        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px;">
            <div>
                <h1 style="font-size: 24px; font-weight: 800; color: #ffffff; margin: 0; text-shadow: 0 1px 2px rgba(0,0,0,0.1);">PicMe225 <span style="color: #ffffff;">Store</span></h1>
                <p style="font-size: 13px; color: rgba(255,255,255,0.95); margin: 4px 0 0 0; font-weight: 500;">Publiez et gérez vos annonces en toute simplicité</p>
            </div>
            <div style="display: flex; gap: 8px;">
                <a href="{{ route('user.marketplace.my') }}" class="btn" style="padding: 8px 16px; background: rgba(255,255,255,0.2); backdrop-filter: blur(4px); color: #ffffff; border-radius: 20px; border: 1px solid rgba(255,255,255,0.3); font-size: 12px; font-weight: 800; text-decoration: none; display: flex; align-items: center; gap: 6px;">
                    <i class="fa fa-list"></i> Mes Annonces
                </a>
                <a href="{{ route('user.marketplace.create') }}" class="btn" style="padding: 8px 16px; background: #ffffff; color: var(--lime-dark, #15803D); border-radius: 20px; font-size: 12px; font-weight: 800; text-decoration: none; display: flex; align-items: center; gap: 6px; box-shadow: 0 2px 8px rgba(0,0,0,0.15);">
    ...
    <section class="pm-mkt-filters">
        <div class="container">
            <form action="{{ route('user.marketplace.explore') }}" method="GET" id="pm-filter-form">
                <!-- Search and Toggle Button -->
                <div style="display:flex; gap:10px; align-items:center;">
                    <div class="pm-search-box" style="flex: 1; min-width: 0;">
                        <i class="fa fa-search"></i>
                        <input type="text" name="search" id="pm-search-input" value="{{ request('search') }}" placeholder="Rechercher un article, un véhicule..." />
                    </div>
                    <button type="button" onclick="document.getElementById('pm-filter-modal').classList.add('show');" style="padding: 12px 16px; background: rgba(201,168,76,0.1); color: #C9A84C; border: 1px solid rgba(201,168,76,0.3); border-radius: 12px; cursor: pointer; white-space: nowrap; font-weight: 700; display:flex; align-items:center; gap:6px;">
            ...
            <div class="pm-cat-chips">
                <div class="pm-cat-chip main-cat {{ !request('category') ? 'active-all' : '' }}" data-cat="">Tout afficher</div>
                @foreach($categories as $c)
                    ...
                    @endphp
                    <div class="pm-cat-chip main-cat {{ $isActiveMain ? 'active' : '' }}" data-cat="{{ $c->name }}">
                        {{ $c->label ?? $c->name }}
                    </div>
                @endforeach
            </div>
            ...
            @if($subcategoriesToShow->count() > 0)
                <div class="pm-cat-chips" id="pm-subcat-container" style="margin-top:10px;">
                    @foreach($subcategoriesToShow as $sub)
                        <div class="pm-cat-chip pm-sub-chip {{ request('category') == $sub->name ? 'active' : '' }}" data-cat="{{ $sub->name }}">
                            {{ $sub->label ?? $sub->name }}
                        </div>
                    @endforeach
                </div>
    ...
    <!-- ═══ FILTER MODAL ═══ -->
    <div class="pm-filter-modal" id="pm-filter-modal">
        <div class="pm-filter-content">
            <button type="button" class="pm-filter-close" onclick="document.getElementById('pm-filter-modal').classList.remove('show');"><i class="fa fa-times"></i></button>
            <h3 style="color:var(--text-dark); margin-bottom:20px; font-weight:800; font-size:20px;">Filtres avancés</h3>
            
            <div style="display:flex; flex-direction:column; gap:16px;">
                <div>
                    <label style="font-size:12px; font-weight:700; color:var(--text-color); margin-bottom:6px; display:block;">Ville / Commune</label>
                    <input type="text" form="pm-filter-form" name="city" value="{{ request('city') }}" placeholder="Ex: Abidjan" style="width:100%; padding:14px; border-radius:12px; border:1px solid var(--border-color); background:rgba(0,0,0,0.03); color:var(--text-dark); outline:none; font-family:'Inter', sans-serif;">
                </div>
                
                <div style="display:flex; gap:12px;">
                    <div style="flex:1;">
                        <label style="font-size:12px; font-weight:700; color:var(--text-color); margin-bottom:6px; display:block;">Prix Min</label>
                        <input type="number" form="pm-filter-form" name="min_price" value="{{ request('min_price') }}" placeholder="0" style="width:100%; padding:14px; border-radius:12px; border:1px solid var(--border-color); background:rgba(0,0,0,0.03); color:var(--text-dark); outline:none; font-family:'Inter', sans-serif;">
                    </div>
                    <div style="flex:1;">
                        <label style="font-size:12px; font-weight:700; color:var(--text-color); margin-bottom:6px; display:block;">Prix Max</label>
                        <input type="number" form="pm-filter-form" name="max_price" value="{{ request('max_price') }}" placeholder="Max" style="width:100%; padding:14px; border-radius:12px; border:1px solid var(--border-color); background:rgba(0,0,0,0.03); color:var(--text-dark); outline:none; font-family:'Inter', sans-serif;">
                    </div>
                </div>
                
                <div>
                    <label style="font-size:12px; font-weight:700; color:var(--text-color); margin-bottom:6px; display:block;">Trier par</label>
                    <select form="pm-filter-form" name="sort" style="width:100%; padding:14px; border-radius:12px; border:1px solid var(--border-color); background:rgba(0,0,0,0.03); color:var(--text-dark); outline:none; font-family:'Inter', sans-serif;">
                ...
                <button type="submit" form="pm-filter-form" style="margin-top:10px; width:100%; padding:16px; background:linear-gradient(135deg, #C9A84C, #e2c06e); color:white; font-weight:800; border:none; border-radius:12px; cursor:pointer; font-size:15px; box-shadow:0 4px 12px rgba(201,168,76,0.3);">Appliquer les filtres</button>
            </div>
        </div>
    </div>
...
(function() {
    ...
    function renderListings(listings) {
        ...
        countEl.textContent = '{{ $listings->total() }}' + ' annonce' + ('{{ $listings->total() }}' > 1 ? 's' : '') + ' trouvée' + ('{{ $listings->total() }}' > 1 ? 's' : '');

        grid.innerHTML = listings.map(function(item) {
            var price = formatPrice(item.price, item.price_unit);
            var isVideo = item.media_url && (item.media_url.toLowerCase().endsWith('.mp4') || item.media_url.toLowerCase().endsWith('.mov') || item.media_url.toLowerCase().endsWith('.avi'));
            var img = '';
            if (item.media_url) {
                if (isVideo) {
                    img = '<video src="' + item.media_url + '" muted style="width:100%;height:100%;object-fit:cover;"></video>';
                } else {
                    img = '<img src="' + item.media_url + '" alt="' + (item.title || '') + '" loading="lazy" style="width:100%;height:100%;object-fit:cover;" onerror="this.parentElement.innerHTML=\'<span style=font-size:48px>\' + catEmoji(item.category) + \'</span>\'">';
                }
            } else {
                img = '<span style="font-size:48px;">' + catEmoji(item.category) + '</span>';
            }
            ...
            var digitalBadge = item.is_digital
                ? '<span style="position:absolute;bottom:10px;left:10px;background:var(--navy, #0D1B2A);color:#fff;border-radius:16px;padding:4px 10px;font-size:11px;font-weight:700;"><i class="fa fa-download"></i> Numérique</span>'
                : '';

            var detailUrl = item.detail_url || ('/user/store/product/' + item.id);
            var whatsappMsg = encodeURIComponent('Bonjour, je suis intéressé(e) par cette annonce : ' + (item.title || '') + ' - ' + window.location.origin + detailUrl);
    ...
    document.getElementById('pm-filter-modal').addEventListener('click', function(e) {
        if (e.target === this) this.classList.remove('show');
    });
    ...
    try {
        ...
        @endphp
        var serverListings = @json($safeListings);
