                ...
                {{ csrf_field() }}
                    <dl class="dl-horizontal left-right">
                        ...
                        @else
                            ...
                            <dd>{{$fare->distance ?? 'N/A'}} Kms</dd>
                            <dt>@lang('user.eta')</dt>
                            <dd>{{$fare->time ?? 'N/A'}}</dd>
                            <dt>@lang('user.estimated_fare') 
                                @if(!empty($fare->round_trip) && $fare->round_trip == 1) (Round Trip) @endif
                            </dt>
                    ...
                    <select class="form-control" name="payment_mode" id="payment_mode" onchange="card(this.value);">
                        ...
                        @if(Setting::get('CARD') == 1 && $cards->count() > 0)
                    ...
                    @if(!empty($fare->surge) && $fare->surge == 1)
                        <span><em>Note : Due to High Demand the fare may vary!</em></span>
                        <div class="surge-block"><span class="surge-text">{{$fare->surge_value ?? 'N/A'}}</span></div>
                    @endif
