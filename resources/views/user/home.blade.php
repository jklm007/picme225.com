@extends('user.layout.user_dashboard')

@section('title', 'Accueil – PicMe225')
...
<div class="pm-app-container">
        ...
        <div class="pm-header-top" style="margin-bottom: 0;">
            <div class="pm-user-greeting">
                <div class="pm-greeting-text">
                    <h1>Bonjour, {{ strtok($user->first_name, ' ') }} 👋</h1>
    ...
    <div class="pm-search-container">
        <a href="{{ url('dashboard') }}" class="pm-search-box">
    ...
    <div class="pm-section">
        ...
        <div class="pm-categories-scroll">
            {{-- Shortcut to Dashboard (VTC focus) --}}
            <a href="{{ url('dashboard') }}" class="pm-category-card">
                <div class="pm-category-icon-wrap">
                    <img src="{{ asset('images/default_category.png') }}" alt="VTC" onerror="this.src='{{ asset('asset/logo.png') }}'">
                </div>
                <span class="pm-category-label">VTC</span>
            </a>
            
            {{-- Loop existing DB categories --}}
            @foreach($categories as $cat)
            @if(strtolower($cat->name) !== 'vtc')
            <a href="{{ url('dashboard') }}" class="pm-category-card">
                <div class="pm-category-icon-wrap">
                    <img src="{{ $cat->image_url ?? asset('images/default_category.png') }}" alt="{{ $cat->name }}" onerror="this.src='{{ asset('asset/logo.png') }}'">
                </div>
                <span class="pm-category-label">{{ $cat->name }}</span>
            </a>
            @endif
            @endforeach
            
            {{-- Marketplace Shortcut --}}
            <a href="{{ route('user.marketplace.explore') }}" class="pm-category-card">
    ...
    <div class="pm-section">
        ...
        <div class="pm-ads-scroll">
            ...
            {{-- STATIC PLACEHOLDER ADS (Fallback) --}}
    ...
    <div class="pm-section">
        <div class="pm-section-header">
            ...
            <a href="{{ route('user.marketplace.explore') }}" class="pm-section-link">Voir tout</a>
        </div>
        <div class="pm-products-scroll">
            @foreach($recentProducts as $prod)
            <a href="{{ route('user.marketplace.detail', $prod->id) }}" class="pm-product-card">
                <img src="{{ $prod->media_url }}" class="pm-product-img" onerror="this.src='{{ asset('asset/logo.png') }}'" alt="{{ $prod->title }}">
                <div class="pm-product-info">
                    <h4 class="pm-product-title">{{ $prod->title }}</h4>
                    <p class="pm-product-price">{{ currency($prod->price) }}</p>
                </div>
            </a>
            @endforeach
        </div>
    ...
    {{-- BOTTOM NAV --}}
    @include('user.include.bottom_nav', ['active' => 'home'])
...
<script>
    ...
    function loadFeed() {
        ...
        fetch('{{ route("user.marketplace.feed") }}?page=' + page)
