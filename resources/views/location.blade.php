        ...
        <div class="row" id="vehicleGrid">
            @forelse($vehicles as $vehicle)
                <div class="col-md-4 col-sm-6 vehicle-item" style="margin-bottom: 40px;" 
                     data-title="{{ strtolower($vehicle->title) }}" 
                     data-city="{{ strtolower($vehicle->location_city) }}"
                     data-brand="{{ strtolower($vehicle->brand) }}">
                    <div class="premium-card">
                        <div class="card-img-container">
                            <img src="{{ url($vehicle->cover_image) }}" alt="{{ $vehicle->title }}">
                            <div class="price-badge">{{ number_format($vehicle->price) }} CFA <span>/ jour</span></div>
                        </div>
                        <div class="card-body-custom">
                            <div class="card-category">{{ $vehicle->brand }} {{ $vehicle->model }}</div>
                            <h4 class="card-title-custom">{{ $vehicle->title }}</h4>
                            <div class="card-info">
                                <span><i class="fa fa-map-marker"></i> {{ $vehicle->location_city }}</span>
                                @if($vehicle->owner_name)
                                    <span><i class="fa fa-user-circle"></i> {{ $vehicle->owner_name }}</span>
                                @endif
                            </div>
                            <div class="card-footer-custom">
                                <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $vehicle->owner_phone ?: '2250101010101') }}?text={{ urlencode('Bonjour ' . ($vehicle->owner_name ?: '') . ', je suis très intéressé par la location de votre véhicule : ' . $vehicle->title . ' (' . $vehicle->brand . ' ' . $vehicle->model . ') à ' . number_format($vehicle->price) . ' CFA/jour vu sur Picme.') }}" 
                                   target="_blank" 
                                   class="btn-whatsapp">
