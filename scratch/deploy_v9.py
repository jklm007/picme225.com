import paramiko, tarfile, time, io
...
with tarfile.open('fixes_v9.tar.gz', 'w:gz') as tar:
...
sftp.put('fixes_v9.tar.gz', '/tmp/fixes_v9.tar.gz')
sftp.close()
...
for label in ['app=laravel', 'app=laravel-worker']:
    ...
    for pod in pods:
        ...
        client.exec_command("kubectl cp /tmp/fixes_v9.tar.gz " + pod + ":/tmp/fixes_v9.tar.gz")
        time.sleep(1)
        client.exec_command("kubectl exec " + pod + " -- tar -xzf /tmp/fixes_v9.tar.gz -C /app")
        time.sleep(1)
        if 'worker' in label:
...
print("\nClearing Groq cache...")
clear_php = open('scratch/clear_groq_cache.php', 'w')
clear_php.write("""<?php
...
clear_php.close()
sftp = client.open_sftp()
sftp.put('scratch/clear_groq_cache.php', '/tmp/clear_groq_cache.php')
sftp.close()
...
print("\nVerifying: no mixtral in code...")
stdin, stdout, stderr = client.exec_command("kubectl exec " + pod + " -- grep -c mixtral /app/app/Jobs/ProcessWhatsappBatchJob.php 2>/dev/null || echo 0")
count = stdout.read().decode().strip()
print("  mixtral references: " + count + " (should be 0)")

print("\nRunning test job...")
test_php = open('scratch/test_job.php', 'w')
test_php.write("""<?php
require '/app/vendor/autoload.php';
$app = require '/app/bootstrap/app.php';
$app->make('Illuminate\\Contracts\\Console\\Kernel')->bootstrap();
...
sftp.close()
client.exec_command("kubectl cp /tmp/test_job.php " + pod + ":/tmp/test_job.php")
time.sleep(1)
stdin, stdout, stderr = client.exec_command("kubectl exec " + pod + " -- php /tmp/test_job.php 2>&1")
