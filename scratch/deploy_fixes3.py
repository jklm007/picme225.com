...
with tarfile.open("fixes_update3.tar.gz", "w:gz") as tar:
    tar.add("app/Models/MarketplaceListing.php")
    tar.add("app/Http/Controllers/InternalUploadController.php")
    tar.add("app/Jobs/ProcessWhatsappBatchJob.php")

sftp = client.open_sftp()
sftp.put("fixes_update3.tar.gz", "/tmp/fixes_update3.tar.gz")
...
for pod in web_pods:
    ...
    client.exec_command(f"kubectl cp /tmp/fixes_update3.tar.gz {pod}:/tmp/fixes_update3.tar.gz")
    client.exec_command(f"kubectl exec {pod} -- tar -xzf /tmp/fixes_update3.tar.gz -C /app")
    client.exec_command(f"kubectl exec {pod} -- php artisan view:clear")
...
# Also delete the phantom listings from the DB
