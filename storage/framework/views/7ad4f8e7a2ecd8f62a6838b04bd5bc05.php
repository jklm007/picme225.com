  ...
  <head>
    ...
    <style>
        ...
        .pm-avatar {
            ...
            background: center/cover url('<?php echo e(img(Auth::user()->picture ?? '')); ?>') no-repeat;
...
<body>
    ...
    <div class="pm-top-bar">
        <div class="pm-user-info" onclick="toggleDrawer()">
            <?php if(Auth::user() && Auth::user()->picture): ?>
                <div class="pm-avatar"></div>
            <?php else: ?>
                <div class="pm-avatar-initials"><?php echo e(substr(Auth::user()->first_name ?? 'P',0,1)); ?><?php echo e(substr(Auth::user()->last_name ?? 'M',0,1)); ?></div>
            <?php endif; ?>
            <div>
                <div class="pm-name"><?php echo e(Auth::user()->first_name ?? 'Invité'); ?> <?php echo e(Auth::user()->last_name ?? ''); ?></div>
                <div class="pm-subtitle">Passager PicMe</div>
            </div>
        </div>
        <div class="pm-top-actions">
            <a href="<?php echo e(url('/wallet')); ?>" class="pm-wallet">
                <i class="fa fa-google-wallet"></i> <?php echo e(currency(Auth::user()->wallet_balance ?? 0)); ?>

            </a>
            <a href="<?php echo e(url('notifications')); ?>" class="pm-notif-btn">
    ...
    <div class="pwa-drawer">
        <div class="pwa-drawer-head">
            <div class="pwa-drawer-avatar-wrap">
                <div class="pwa-drawer-avatar" style="background-image: url('<?php echo e(img(Auth::user()->picture ?? '')); ?>');">
                    <?php if(!Auth::user() || !Auth::user()->picture): ?>
                        <?php echo e(substr(Auth::user()->first_name ?? 'P',0,1)); ?><?php echo e(substr(Auth::user()->last_name ?? 'M',0,1)); ?>

                    <?php endif; ?>
                </div>
                <div>
                    <div class="pwa-drawer-user-name"><?php echo e(Auth::user()->first_name ?? 'Invité'); ?> <?php echo e(Auth::user()->last_name ?? ''); ?></div>
                    <div class="pwa-drawer-user-email"><?php echo e(Auth::user()->email ?? ''); ?></div>
                </div>
            </div>
            <div class="pwa-drawer-wallet-badge">
                <i class="fa fa-leaf" style="color: #27ae60;"></i> <?php echo e(Auth::user()->eco_token_balance ?? '0'); ?> ECO
            </div>
        </div>
        <div class="pwa-drawer-menu">
            ...
            <a href="<?php echo e(url('trips')); ?>" class="<?php echo e(Request::is('trips') ? 'active' : ''); ?>">
                <div class="drawer-icon"><i class="fa fa-history"></i></div> <?php echo app('translator')->get('user.my_trips'); ?>
            </a>
            <a href="<?php echo e(url('wallet')); ?>" class="<?php echo e(Request::is('wallet') ? 'active' : ''); ?>">
                <div class="drawer-icon"><i class="fa fa-google-wallet"></i></div> <?php echo app('translator')->get('user.my_wallet'); ?>
            </a>
            <a href="<?php echo e(url('promotions')); ?>" class="<?php echo e(Request::is('promotions') ? 'active' : ''); ?>">
                <div class="drawer-icon"><i class="fa fa-tag"></i></div> <?php echo app('translator')->get('user.promotion'); ?>
            </a>
            <div class="pwa-drawer-sep"></div>
            <a href="<?php echo e(url('profile')); ?>">
                <div class="drawer-icon"><i class="fa fa-user"></i></div> <?php echo app('translator')->get('user.profile.profile'); ?>
            </a>
            <a href="<?php echo e(url('change/password')); ?>">
                <div class="drawer-icon"><i class="fa fa-lock"></i></div> <?php echo app('translator')->get('user.profile.change_password'); ?>
            </a>
            <div class="pwa-drawer-sep"></div>
            <div class="pwa-drawer-logout">
                <button onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
                    <div class="drawer-icon"><i class="fa fa-sign-out"></i></div> <?php echo app('translator')->get('user.profile.logout'); ?>
                </button>
            </div>
        </div>
    </div>
    ...
    <div class="page-content dashboard-page">    
        <div class="container pm-wrapper-content" style="height: 100%;">
            <?php echo $__env->yieldContent('content'); ?>
        </div>
    </div>

    <!-- BOTTOM NAV -->
    <?php echo $__env->make('user.include.bottom_nav', ['active' => $bottomNavActive ?? $active ?? ''], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
                ...
                success: function(data) {
                    if (data && data.type === 'PRIVATE') {
                        ...
                        if (data.target_url) {
                            ...
                            $('#pm-ad-link').on('click', function(e) {
                                $.post('/api/user/ad/click', { campaign_id: data.campaign_id, _token: "<?php echo e(csrf_token()); ?>" });
                                // Intercept for internal routing
                                if (data.target_url) {
                                    if (data.target_url.indexOf('picme225.site') !== -1 || 
                                        data.target_url.indexOf(window.location.host) !== -1 || 
                                        data.target_url.startsWith('/')) {
                                        e.preventDefault();
                                        $('#pm-global-ad-popup').modal('hide');
                                        window.location.href = data.target_url;
                                    } else if (!isNaN(data.target_url) && data.target_url.trim() !== '') {
                                        // If target_url is just an ID, redirect to marketplace detail
                                        e.preventDefault();
                                        $('#pm-global-ad-popup').modal('hide');
                                        window.location.href = '/user/store/product/' + data.target_url;
                                    }
                                }
                            });
                        } else {
    ...
    <?php echo $__env->make('common.pwa_installer', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
</body>
</html>
<?php /**PATH C:\Users\HP\Documents\Jews-world Backend\picme225.com_backend\resources\views/user/layout/user_dashboard.blade.php ENDPATH**/ ?>