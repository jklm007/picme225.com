...
<div class="pm-login-wrapper">
    ...
    <div class="pm-login-logo">
        <?php if(Setting::get('site_logo')): ?>
            <img src="<?php echo e(img(Setting::get('site_logo'))); ?>" alt="<?php echo e(Setting::get('site_title', 'PicMe225')); ?>">
        <?php else: ?>
            <img src="<?php echo e(asset('logo.png')); ?>" alt="PicMe225">
        <?php endif; ?>
        <span class="pm-brand-name"><?php echo e(Setting::get('site_title', 'PicMe225')); ?></span>
    ...
    <div class="pm-login-card">
        ...
        <div class="pm-tabs" id="pm-tabs">
            <button class="pm-tab <?php if(!$errors->has('login_type')): ?> active <?php endif; ?>"
                    data-pane="admin" onclick="switchTab(this, 'admin')">
                <span class="pm-tab-icon">🛡️</span>Admin
            </button>
            <button class="pm-tab <?php if($errors->has('login_type') && $errors->first('login_type') == 'dispatcher'): ?> active <?php endif; ?>"
                    data-pane="dispatcher" onclick="switchTab(this, 'dispatcher')">
                <span class="pm-tab-icon">📡</span>Dispatcher
            </button>
            <button class="pm-tab <?php if($errors->has('login_type') && $errors->first('login_type') == 'fleet'): ?> active <?php endif; ?>"
                    data-pane="fleet" onclick="switchTab(this, 'fleet')">
                <span class="pm-tab-icon">🚗</span>Flotte
            </button>
            <button class="pm-tab <?php if($errors->has('login_type') && $errors->first('login_type') == 'account'): ?> active <?php endif; ?>"
        ...
        <div class="pm-tab-content">
            ...
            <?php if(session('flash_success')): ?>
                <div class="pm-flash pm-flash-success">
                    <i class="fa fa-check-circle"></i> <?php echo e(session('flash_success')); ?>

                </div>
            <?php endif; ?>
            <?php if(session('flash_error') || $errors->has('login')): ?>
                <div class="pm-flash pm-flash-error">
                    <i class="fa fa-exclamation-circle"></i>
                    <?php echo e(session('flash_error') ?? $errors->first('login') ?? 'Identifiants incorrects.'); ?>

                </div>
            <?php endif; ?>

            <!-- ── ADMIN ── -->
            <div class="pm-tab-pane <?php if(!$errors->has('login_type')): ?> active <?php endif; ?>" id="pane-admin">
                ...
                <div class="pm-forgot">
                    <a href="<?php echo e(url('/admin/password/reset')); ?>">Mot de passe oublié ?</a>
                </div>
            </div>

            <!-- ── DISPATCHER ── -->
            <div class="pm-tab-pane <?php if($errors->has('login_type') && $errors->first('login_type') == 'dispatcher'): ?> active <?php endif; ?>" id="pane-dispatcher">
                <div class="pm-form-title">Connexion Dispatcher</div>
                <div class="pm-form-subtitle">Gestion des répartitions et affectations</div>
                <form method="POST" action="<?php echo e(url('/dispatcher/login')); ?>">
                    <?php echo e(csrf_field()); ?>
            ...
            <!-- ── FLEET ── -->
            <div class="pm-tab-pane <?php if($errors->has('login_type') && $errors->first('login_type') == 'fleet'): ?> active <?php endif; ?>" id="pane-fleet">
                <div class="pm-form-title">Connexion Flotte</div>
                <div class="pm-form-subtitle">Gestion de la flotte de véhicules</div>
                <form method="POST" action="<?php echo e(url('/fleet/login')); ?>">
                    <?php echo e(csrf_field()); ?>
            ...
            <!-- ── ACCOUNT ── -->
            <div class="pm-tab-pane <?php if($errors->has('login_type') && $errors->first('login_type') == 'account'): ?> active <?php endif; ?>" id="pane-account">
                ...
                <form method="POST" action="<?php echo e(url('/account/login')); ?>">
                    <?php echo e(csrf_field()); ?>
    ...
    <div class="pm-login-footer">
        ...
        Connexion sécurisée &mdash; <span>PicMe225</span> &copy; <?php echo e(date('Y')); ?>
...
<?php echo $__env->make('admin.layout.auth', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /app/resources/views/admin/auth/login.blade.php ENDPATH**/ ?>