...
class UserApiController extends Controller
{
    ...
    public function send_request(Request $request) {
        ...
        try{
            ...
            $UserRequest->method = $request->method;
    ...
    public function estimated_fare(Request $request){
        ...
        try{
            ...
            $package = isset($request->package_id) ? $request->package_id : 0;
            $rental_price = '';
            if($package != 0){
                $pack_val= KmHour::findorfail((int)$package);
                $service_rental =ServiceTypeRental::where('service_type_id',$request->service_type)->where('km_hour_id',(int)$package)->get();
                $rental_price = count($service_rental) != 0 ? $service_rental[0]->ren_price : '';
                $package = $pack_val->kilometer.'Kms- '.$pack_val->hour.'Hrs';
            } 
            ...
            if($request->has('method') && $request->method == 'outstation'){
                $price = ($kilometer * $service_type->outstation_price);
                if($request->round_trip == 1){
                    $total = ($price * 2) + $tax_price; 
                }else{
                    $total = $price + $tax_price; 
                }
            }
            ...
            */ 
            ...
            return response()->json([
                    ...
                    'rental_amount' => $rental_price,
                    'package' => $package,
                    'method' => $request->method,
                    'round_trip' => @$request->has('round_trip') ? $request->round_trip : ''
                ]);
    ...
    public function promo_passbook(Request $request)
    {
        try{
            ...
            return PromocodeUsage::where('user_id',Auth::user()->id)->with('promocode')->get();
