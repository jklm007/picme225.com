...
files_to_upload = {
    ...
    r'prod_setup_slots.php': '/tmp/prod_setup_slots.php',
}
...
for POD in $LARAVEL_PODS; do
    ...
    kubectl cp /tmp/prod_setup_slots.php default/$POD:/app/prod_setup_slots.php
    kubectl exec $POD -- php artisan view:clear
    kubectl exec $POD -- php /app/prod_setup_slots.php
    echo "Deployed to $POD"
done
"""
print("Running kubectl cp and script on pods...")
